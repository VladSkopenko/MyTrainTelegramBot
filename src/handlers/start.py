...
async def get_start_and_choice_lang(message: Message, bot: Bot, ):
    ...
    await bot.send_message(message.from_user.id, "Оберіть мову:",
...
async def language_callback(callback_query: CallbackQuery):
    ...
    await callback_query.message.answer(f"{callback_query.from_user.first_name}, Авторизуйтесь будь-ласка",
